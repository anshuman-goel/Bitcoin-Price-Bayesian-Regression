...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
        mean_b=np.mean(Xi.iloc[i][:-1])
        std_b=np.std(Xi.iloc[i][:-1])
        num=0
        for j in range(len(X[:-1])):
            num+=(X[j]-mean_a)*(Xi.iloc[i][j]-mean_b)
        num=float(num)/float(len(X[:-1])*std_a*std_b)
        eq_num+=Xi.iloc[i][-1]*np.exp(wt*num)
        eq_den+=np.exp(wt*num)
    return float(eq_num)/float(eq_den)
